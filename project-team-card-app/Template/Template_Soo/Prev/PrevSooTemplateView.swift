...
class PrevSooTemplateView: UIView {
    // 탑 버튼 크기 (편집, 삭제)
...
//    
    ...
    // 커스텀 정보 테이블 뷰
    public let customInfoTableView: UITableView = {
        let view = UITableView()
        view.register(SooCustomInfoCell.self, forCellReuseIdentifier: SooCustomInfoCell.id)
        view.estimatedRowHeight = 60 // 예상 높이 (적절한 값으로 설정)
        view.rowHeight = UITableView.automaticDimension // 자동 계산
...
//            introduceView.bottomAnchor.constraint(equalTo: contentView.bottomAnchor),
            ...
            // 테이블 뷰
            customInfoTableView.topAnchor.constraint(equalTo: introduceView.bottomAnchor, constant: 15),
            customInfoTableView.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: 16),
            customInfoTableView.trailingAnchor.constraint(equalTo: self.trailingAnchor, constant: -16),
            customInfoTableView.bottomAnchor.constraint(equalTo: contentView.bottomAnchor),
//            customInfoTableView.heightAnchor.constraint(equalToConstant: 100)
